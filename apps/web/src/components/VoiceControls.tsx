...
export const VoiceControls = ({
  ...
  useEffect(() => {
    ...
    if (pending.symbol !== currentSymbol) {
  ...
  const executeAiIntent = (intent: AiIntent): void => {
    ...
    if (intent.action === "START_REALTIME") {
      startSession(
        symbol,
        getStockName(symbol),
        intent.thresholdRate != null
          ? "alerts-only"
          : intent.metrics.includes("volume")
            ? "price-volume"
            : intent.metrics.length
              ? "price-only"
              : preferences.mode,
        intent.thresholdRate
      );
      return;
    }
    ...
    if (intent.action === "REPLAY_RECENT") {
      const seconds = replaySeconds(intent.timeRange);

      if (!seconds) {
        speakAndShow("최근 1분, 3분, 5분 중 선택해 주세요.");
        return;
      }

      executeSonificationPlan({
    ...
    if (intent.action === "REPLAY_LAST") {
      if (lastSoundEvent?.symbol !== symbol) {
        speakAndShow("해당 종목의 최근 소리 기록이 없습니다.");
        return;
      }
  ...
  const handleCommand = async (transcript: string): Promise<void> => {
    ...
    if (
      pendingQuestion ||
      local.intent === "set-preferences" ||
      local.intent === "explain-last-sound" ||
    ...
    try {
      const context = {
        currentSymbol,
        currentStockName: getStockName(currentSymbol),
        ...(previousUtterance.current
          ? { previousUtterance: previousUtterance.current }
          : {})
      };
      ...
      if (intent && intent.action !== "UNKNOWN") {
        executeLatest.current(intent);
      } else {
        fallbackLatest.current(transcript);
      }
    } catch {
      if (requestRef.current === controller) {
        fallbackLatest.current(transcript);
