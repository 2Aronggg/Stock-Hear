...
export const AudioControls = ({
  ...
  <section className="panel audio-controls" aria-labelledby="audio-controls-title">
    ...
    <div className="mixer-stack" aria-label="소리 크기 조절">
      <div className="mixer-row">
        <div className="mixer-row-heading">
          <span>소리 크기</span>
          <output>{Math.round((volume / 0.4) * 100)}%</output>
        </div>

        <div className="mixer-control">
          <span aria-hidden="true">작게</span>
          <input
            type="range"
            min="0"
            max="0.4"
    ...
    <div className="button-row" aria-label="등락 소리 샘플">
      ...
      <button
        className="sample-button down"
        type="button"
        onClick={() => onSample("down")}
      >
        하락음
      </button>

      <button
        className="sample-button flat"
        type="button"
        onClick={() => onSample("flat")}
      >
        보합음
      </button>
    </div>
